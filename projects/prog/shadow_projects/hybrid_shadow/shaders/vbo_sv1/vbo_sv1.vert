...
//			f_weights[i]	= 0.5;
		...
		if ( b_edges_are_silhouettes[0] && b_edges_are_silhouettes[1])
		{
			...
			{
				v3_edge[1] = v3_vertex[0] + (v3_vertex[1] - v3_vertex[0]) * (f_weights[0] / (f_weights[0] + f_weights[1]));
				v3_proj_edge[1] = v3_edge[1] - f_coef_extrusion*normalize(v3_light_position_in_object - v3_edge[1]);
				v3_position_vertex = v3_proj_edge[1];
//				v_v4_color = GREEN;
			}
			else
			{
				v3_edge[0] = v3_vertex[0] + (v3_vertex[2] - v3_vertex[0]) * (f_weights[0] / (f_weights[0] + f_weights[2]));
				v3_proj_edge[0] = v3_edge[0] - f_coef_extrusion*normalize(v3_light_position_in_object - v3_edge[0]);
...
//				v_v4_color = RED;
		...
		else if ( !b_edges_are_silhouettes[0] && b_edges_are_silhouettes[1])
		{
			...
			{
				v3_edge[0] = v3_vertex[2] + (v3_vertex[1] - v3_vertex[2]) * (f_weights[2] / (f_weights[1] + f_weights[2]));
				v3_position_vertex = v3_edge[0];
//				v_v4_color = RED;
			}
			else
			{
				v3_edge[1] = v3_vertex[2] + (v3_vertex[0] - v3_vertex[2]) * (f_weights[2] / (f_weights[0] + f_weights[2]));
...
//				v_v4_color = GREEN;
		...
		else // b_edges_are_silhouettes[0] && !b_edges_are_silhouettes[1] == TRUE
		{
			...
			{
				v3_edge[1] = v3_vertex[1] + (v3_vertex[2] - v3_vertex[1]) * (f_weights[1] / (f_weights[1] + f_weights[2]));
				v3_position_vertex = v3_edge[1];
//				v_v4_color = GREEN;
			}
			else
			{
				v3_edge[0] = v3_vertex[1] + (v3_vertex[0] - v3_vertex[1]) * (f_weights[1] / (f_weights[0] + f_weights[1]));
				v3_position_vertex = v3_edge[0];
//				v_v4_color = RED;
			}
		}
