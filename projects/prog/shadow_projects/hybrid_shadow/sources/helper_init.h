...
void Viewer::initShaders( bool bDestroy )
{
	...
	prog_vbo_sv_geom.loadDir(
                PARAM(QString, shaderDir.vbo_sv_geom),
		false,							// <subdir>
		true, 							// load geometry shader
		PARAM(GLenum, 	vbo.geometry_shader.input_type),	// specify input type for geometry shader
		PARAM(GLenum, 	vbo.geometry_shader.output_type),	// specify output type for geometry shader
		PARAM(GLint, 	vbo.geometry_shader.nb_vertices_out)	// specify max output vertices
	);

        prog_vbo_smoothies_geom.loadDir(
                    PARAM(QString, shaderDir.vbo_smoothies_geom),
                    false,							// <subdir>
                    true, 							// load geometry shader
                    PARAM(GLenum, 	vbo.smoothies.geometry_shader.input_type),	// specify input type for geometry shader
                    PARAM(GLenum, 	vbo.smoothies.geometry_shader.output_type),	// specify output type for geometry shader
                    PARAM(GLint, 	vbo.smoothies.geometry_shader.nb_vertices_out)	// specify max output vertices
            );
...
void Viewer::initTriSoup( bool bDestroy )
{
	...
	// [OK]: geom shader, [FAILED]: sinon
	//ts = TriSoup::load( PARAM(QString, model.default));
	//
	// [OK]: geom shader, [OK]: vbo expand (informations)
	ts = TriSoup2::load( PARAM(QString, model.default), !PARAM(bool, shadow_volume.use_geometry_shader) );
	//
	//ts = TriSoup::load( PARAM(QString, model.default), PARAM(bool, vbo.expand) ); // true => expand	// OLD
	//ts = TriSoup::load( PARAM(QString, model.default), !PARAM(bool, shadow_volume.use_geometry_shader) ); // OLD
